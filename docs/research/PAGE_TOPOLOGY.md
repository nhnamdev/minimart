...
1. Store header — fixed-height visual block with blurred avatar background, logo, store name, opening hours, language control, phone action, and search action.
2. Order tabs — two equal-width items: `Đặt hàng` (active) and `Đơn hàng`.
3. Catalog workspace — fills the remaining viewport above the 48px cart bar.
...
4. Shopping cart — fixed to the viewport bottom, dark background, yellow active state, total and minimum-order copy.
5. Overlays — language selector, search UI, product detail, and cart summary are click-driven layers above the page.
...
- Cart: click-driven summary drawer; checkout remains demo-only.
